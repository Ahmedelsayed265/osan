  ...
  zid.account
    ...
    .then((response) => {
      if (response) {
        ...
        const filledButton = container.find(
          `[zid-visible-wishlist="${productId}"]`,
        )[0];
        const emptyButton =
          container.find(`[zid-hidden-wishlist="${productId}"]`)[0] ||
          container.find(".icon-heart-mask:not([zid-visible-wishlist])")[0];

        if (filledButton) {
          filledButton.style.setProperty(
            "display",
            "inline-block",
            "important",
          );
...
function removeFromWishlist(elm, productId) {
  ...
  zid.account
    .removeFromWishlist(productId, { showErrorNotification: true })
    .then((response) => {
      container.find(".loader").addClass("d-none");
...
function shareWishlist() {
  $(".share-wishlist .loader")
    .removeClass("d-none")
    .siblings(".share-icon")
    .addClass("d-none");

  zid.account
    .shareWishlist({ showErrorNotification: true })
    .then(async (response) => {
      if (response) {
        $(".share-wishlist .loader")
          .addClass("d-none")
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(document).on("click", "#sliding-menu .clickable-span", function (e) {
    ...
    .then(function (response) {
      if (response && response.results) {
        const resultsContainer = $(target)
          .closest(".search-content, .sm-search-div, .autocomplete")
...
class ProductsQuestions {
  ...
  async submitQuestion(productId) {
    ...
    if (isValid) {
      ...
      try {
        const response = await zid.products.createQuestion(
          productId,
          {
...
function updateUIAfterLogin(customer) {
  ...
  if (
    loyaltySection &&
    !loyaltySection.classList.contains("loyalty-points-section-d-none")
  ) {
    location.reload();
    return;
  }
  ...
  document
    .querySelectorAll(".add-to-wishlist > a.icon-heart-mask")
    .forEach((el) => {
      el.style.setProperty("display", "none", "important");
    });

  document.querySelectorAll(".add-to-wishlist").forEach((container) => {
    const customerSpan = container.querySelector(
      "span:not([zid-visible-guest])",
    );
    if (customerSpan) {
    ...
    window.zid.account
      ...
      .then((wishlistResponse) => {
        ...
        document.querySelectorAll(".add-to-wishlist").forEach((container) => {
          ...
          const emptyButton =
            container.querySelector(`[zid-hidden-wishlist="${productId}"]`) ||
            container.querySelector(
              ".icon-heart-mask:not([zid-visible-wishlist])",
            );
  ...
  setTimeout(() => {
    ...
    document.querySelectorAll(".add-to-wishlist").forEach((container) => {
      ...
      if (
        customerSpan &&
        window.getComputedStyle(customerSpan).display === "none"
      ) {
        customerSpan.style.setProperty("display", "inline-block", "important");
      }
    });
